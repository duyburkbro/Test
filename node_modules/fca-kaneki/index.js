...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`Đăng Nhập Tại ID: ${userID}`, "[ FCA-KANEKI ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`Vùng Của Tài Khoản Là: ${region}`, "[ FCA-KANEKI ]");
    } else {
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function(res) {
        ...
        logger("Đang Đăng Nhập...", "[ FCA-KANEKI ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger("Bạn Đang Bật 2 Bảo Mật !", "[ FCA-KANEKI ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger("Xác Nhận Từ Trình Duyệt, Đang Đăng Nhập...", "[ FCA-KANEKI ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
try { 
    if (appState) {
       ...
       // const os = require("os");
        ...
        // prompt: chalk.hex('#00CCCC').bold('[FCA-KANEKI] • ')
        // });
        // let type = {
        //     1: {    
        ...
        console.log(e);
        logger("Đã Có Lỗi Khi Đang Try Tạo Random Password =))", "[ FCA-KANEKI ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger('Chưa Sẵn Sàng Để Decrypt Appstate !', '[ FCA-KANEKI ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger('Decrypt Appstate Thành Công !', '[ FCA-KANEKI ]');
                }
                catch (e) {
                    logger('Thay AppState Đi !', '[ FCA-KANEKI ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger('Thay AppState Đi !', '[ FCA-KANEKI ]')
        }
    }
    try { 
    ...
    return logger('Thay AppState Đi !', '[ FCA-KANEKI ]')
        ...
        console.log(e);
            ...
            mainPromise
                .then(function() {
                    logger('Hoàn Thành Quá Trình Đăng Nhập !', "[ FCA-KANEKI ]");
                        logger('Auto Check Update ...', "[ FCA-KANEKI ]");
                            //!---------- Auto Check, Update START -----------------!//
                        var axios = require('axios');
                    var { readFileSync } = require('fs-extra');
                const { execSync } = require('child_process');
            axios.get('https://pastebin.com/raw/u09fqR8C').then(async (res) => {
                const localbrand = JSON.parse(readFileSync('./node_modules/fca-kaneki/package.json')).version;
                    if (localbrand != res.data.version) {
                        log.warn("[ FCA-KANEKI ] •",`Phiên Bản Mới Đã Được Publish: ${JSON.parse(readFileSync('./node_modules/fca-kaneki/package.json')).version} => ${res.data.version}`);
                        log.warn("[ FCA-KANEKI ] •",`Tiến Hành Tự Động Cập Nhật Lên Phiên Bản Mới Nhất !`);
                            try {
                                execSync('npm install fca-kaneki@latest', { stdio: 'inherit' });
                                logger("Nâng Cấp Phiên Bản Thành Công!","[ FCA-KANEKI ]")
                                logger('Đang Khởi Động Lại...', '[ FCA-KANEKI ]');
                            ...
                            log.warn('Lỗi Auto Update ! ' + err);
                                ...
                                /*try {
                                    ...
                                    if (data) {
                                        logger.onLogger('Đã Gửi Báo Cáo Lỗi Tới Server !', '[ FCA-KANEKI ]'," #FF0000")
                                    }
                                }
                                catch (e) {
                                    logger.onLogger('Đã Xảy Ra Lỗi Khi Cố Gửi Lỗi Đến Server', '[ FCA-KANEKI ]'," #FF0000")
                                }*/
                            
                        }
                    }
                        else { 
                            logger(`Bạn Hiện Đang Sử Dụng Phiên Bản: ` + localbrand + ' !', "[ FCA-KANEKI ]"); 
